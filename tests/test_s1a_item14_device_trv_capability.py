...
# ── Temperature-Unit audit: Fahrenheit-system min/max plausibility ──────────
#
# min_temp/max_temp are HA climate capability_attributes — already normalized
# to hass.config.units.temperature_unit by HA's own climate platform (see
# homeassistant.helpers.temperature.display_temp), so on a Fahrenheit-system
# installation these attributes are genuinely Fahrenheit values (e.g. 41°F,
# not 5°C). Before the fix, "0 < dev_min <= 30" silently failed open for such
# values (41 > 30), disabling the plausibility clamp entirely.

class TestResolveDeviceEffectiveSetpointFahrenheitSystem:
    def _coord_fahrenheit(self):
        from homeassistant.const import UnitOfTemperature
        coord = make_coordinator()
        coord.hass.config.units.temperature_unit = UnitOfTemperature.FAHRENHEIT
        return coord

    def test_min_temp_clamp_still_applies_in_fahrenheit_system(self):
        """41°F (=5°C) must still clamp — not silently disabled by the Fahrenheit value."""
        coord = self._coord_fahrenheit()
        state = _trv_state(min_temp=41.0, max_temp=86.0, step=None)  # 5°C..30°C
        result = coord.resolve_device_effective_setpoint("climate.trv", state, 3.0)
        assert result == pytest.approx(5.0, abs=0.1)

    def test_max_temp_clamp_still_applies_in_fahrenheit_system(self):
        """86°F (=30°C) ceiling must still clamp."""
        coord = self._coord_fahrenheit()
        state = _trv_state(min_temp=41.0, max_temp=86.0, step=None)
        result = coord.resolve_device_effective_setpoint("climate.trv", state, 32.0)
        assert result == pytest.approx(30.0, abs=0.1)

    def test_within_range_unaffected_in_fahrenheit_system(self):
        coord = self._coord_fahrenheit()
        state = _trv_state(min_temp=41.0, max_temp=86.0, step=None)
        result = coord.resolve_device_effective_setpoint("climate.trv", state, 21.0)
