"""Proves the LE 2.0 BoostModel is pure (no HA / storage / coordinator / sibling models)."""
...
root = types.ModuleType("ts_p14")
root.__path__ = [str(learning_dir)]
sys.modules["ts_p14"] = root
mpkg = types.ModuleType("ts_p14.models")
mpkg.__path__ = [str(models_dir)]
sys.modules["ts_p14.models"] = mpkg

for name in ("clock", "contracts", "raw_schemas", "episode_schemas", "features", "registry"):
    spec = importlib.util.spec_from_file_location(f"ts_p14.{name}", learning_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p14.{name}"] = mod
    spec.loader.exec_module(mod)

for name in ("base", "boost"):
    spec = importlib.util.spec_from_file_location(f"ts_p14.models.{name}", models_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p14.models.{name}"] = mod
    spec.loader.exec_module(mod)
...
def test_boost_imports_without_home_assistant():
...
def test_boost_source_no_forbidden_imports():
    src = (MODELS_DIR / "boost.py").read_text(encoding="utf-8")
    for pat in (
        ...
        assert not pat.search(src), f"boost.py matches forbidden import {pat.pattern!r}"
