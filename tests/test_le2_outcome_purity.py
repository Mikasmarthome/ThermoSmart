"""Proves the LE 2.0 OutcomeModel is pure (no HA / storage / coordinator / sibling models)."""
...
root = types.ModuleType("ts_p11")
root.__path__ = [str(learning_dir)]
sys.modules["ts_p11"] = root
mpkg = types.ModuleType("ts_p11.models")
mpkg.__path__ = [str(models_dir)]
sys.modules["ts_p11.models"] = mpkg

for name in ("clock", "contracts", "raw_schemas", "episode_schemas", "features", "registry"):
    spec = importlib.util.spec_from_file_location(f"ts_p11.{name}", learning_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p11.{name}"] = mod
    spec.loader.exec_module(mod)

for name in ("base", "outcome"):
    spec = importlib.util.spec_from_file_location(f"ts_p11.models.{name}", models_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p11.models.{name}"] = mod
    spec.loader.exec_module(mod)
...
def test_outcome_imports_without_home_assistant():
...
def test_outcome_source_no_forbidden_imports():
    src = (MODELS_DIR / "outcome.py").read_text(encoding="utf-8")
    for pat in (
        ...
        assert not pat.search(src), f"outcome.py matches forbidden import {pat.pattern!r}"
