"""Proves the LE 2.0 HeatRateModel is pure (no HA / storage / coordinator)."""
...
root = types.ModuleType("ts_p8")
root.__path__ = [str(learning_dir)]
sys.modules["ts_p8"] = root
mpkg = types.ModuleType("ts_p8.models")
mpkg.__path__ = [str(models_dir)]
sys.modules["ts_p8.models"] = mpkg

for name in ("clock", "contracts", "raw_schemas", "episode_schemas", "features", "registry"):
    spec = importlib.util.spec_from_file_location(f"ts_p8.{name}", learning_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p8.{name}"] = mod
    spec.loader.exec_module(mod)

for name in ("base", "heat_rate"):
    spec = importlib.util.spec_from_file_location(f"ts_p8.models.{name}", models_dir / f"{name}.py")
    mod = importlib.util.module_from_spec(spec)
    sys.modules[f"ts_p8.models.{name}"] = mod
    spec.loader.exec_module(mod)
...
def test_heat_rate_imports_without_home_assistant():
...
def test_model_sources_have_no_forbidden_imports():
    ...
    pat_coord = re.compile(r"^\s*from\s+\.\.coordinator", re.MULTILINE)
    for path in MODELS_DIR.glob("*.py"):
        ...
        assert not pat_coord.search(src), f"{path.name} imports coordinator"
