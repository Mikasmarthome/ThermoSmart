...
class TestLE1FrozenSemantics:
    ...
    @pytest.mark.asyncio
    async def test_coordinator_calls_le1_observe_in_all_modes(self):
        """Coordinator calls learning_engine.async_observe in ALL four modes.

        The call always happens — LE1's internal frozen check is what no-ops it.
        This test verifies the coordinator does NOT gate LE1 by _learning_mode_on.
        """
        for lm, ac in [(False, False), (True, False), (False, True), (True, True)]:
            coord = _base_coord(lm, ac)
            # async_observe is a MagicMock AsyncMock
            await _cycle(coord)
            assert coord.learning_engine.async_observe.called, (
                f"le1.async_observe not called in mode ({lm},{ac})")
            coord.learning_engine.async_observe.reset_mock()
