...
def test_t17_no_live_call_site_for_record_method():
    ...
    ha_integration.py — checked here as a snapshot of that boundary. Two
    later, separately-approved steps legitimately added call sites:
    ("Research Daily Aggregation Increment 1")
    ...
    newly-appended Support Critical Event), and
    ("Research Daily Aggregation Increment 2")
    _maybe_record_research_daily_progress_sample_safe() (itself only
    reached from learning_progress_safe()'s successful
    compute_learning_progress() branch). Both are expected, intentional
    aggregation of already-produced signals, not a runtime/coordinator hook
    and not a regression of this step's own scope. What must remain true
    regardless is that no runtime/coordinator/control path calls
    record_research_daily_observation_safe() directly (see
    test_le2_research_daily_support_event_aggregation.py's and
    test_le2_research_daily_progress_confidence_aggregation.py's own T21/T13
    checks)."""
    import custom_components.thermosmart.learning.runtime.ha_integration as _ha
