...
def make_mock_hass() -> MagicMock:
    ...
    hass.config.units.temperature_unit defaults to Celsius — the system unit
    every existing test in this suite already assumes (comfort_temp=21.0
    etc.). Real Home Assistant always provides a valid UnitOfTemperature
    string here; a bare MagicMock would make to_internal_temperature_c()'s
    system-unit fallback silently reject every climate-attribute read (no
    explicit unit_of_measurement) as an unrecognized unit.
    """
